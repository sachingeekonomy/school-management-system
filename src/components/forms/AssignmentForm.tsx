  ...
  return (
    <form className="flex flex-col gap-6 max-h-[80vh] overflow-y-auto px-6 py-4" onSubmit={onSubmit}>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              >
                ...
                {lessons?.map((lesson: { id: number; name: string; subject: { name: string }; class: { name: string }; teacher: { name: string; surname: string } }) => (
                  <option value={lesson.id} key={lesson.id}>
                    {lesson.name} - {lesson.subject.name} - {lesson.class.name} ({lesson.teacher.name} {lesson.teacher.surname})
                  </option>
                ))}
              </select>
        ...
        <div>
          <span className="text-xs text-gray-400 font-medium">Date Information</span>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            <InputField 
              label="Start Date" 
              name="startDate" 
